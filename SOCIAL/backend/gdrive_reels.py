...
===================================================================
✅ FALLBACK FOR EVERY STEP - Never fails completely
✅ 1.4x voiceover speed (increased from 1.05x)
...
# ═══════════════════════════════════════════════════════════════════════
# VOICEOVER WITH 1.4X SPEED - ELEVENLABS PRIORITY + EDGE TTS FALLBACK
# ═══════════════════════════════════════════════════════════════════════

async def generate_voiceover_12x(script: str, output: str) -> tuple[bool, str]:
    """
    Generate voiceover at 1.4x speed with priority system:
    1. ElevenLabs (PRIMARY)
    2. Edge TTS (FALLBACK)
    """
    logger.info("🎙️ Voiceover (1.4x speed)...")
    log_memory("voice-start")
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=60) as client:
                ...
                if resp.status_code == 200:
                    ...
                    # Apply 1.4x speed
                    if run_ffmpeg([
                        "ffmpeg", "-i", base, "-filter:a", "atempo=1.4",
                        "-y", output
                    ], 30):
                        cleanup(base)
                        size = os.path.getsize(output) / 1024
                        logger.info(f"✅ ElevenLabs Voiceover (1.4x): {size:.1f}KB")
                        log_memory("voice-done")
                        return True, ""
                    cleanup(base)
        except Exception as e:
    ...
    try:
        ...
        if run_ffmpeg([
            ...
            cleanup(base)
            size = os.path.getsize(output) / 1024
            logger.info(f"✅ Edge TTS Voiceover (1.4x): {size:.1f}KB")
            log_memory("voice-done")
            return True, ""
...
async def process_reel(drive_url: str, user_id: str, task_id: str):
    ...
    try:
        ...
        # Voiceover at 1.4x (ElevenLabs → Edge TTS fallback)
        update(60, "Voiceover (1.4x speed)...")
        voiceover = os.path.join(temp_dir, "voice.mp3")
...
async def initialize():
    ...
    logger.info("="*80)
    logger.info("✅ 1.4x voiceover speed")
