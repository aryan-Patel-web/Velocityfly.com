...
🎬 PROFESSIONAL YOUTUBE SHORTS AD GENERATOR
================================================
✅ YouTube Shorts compliant
✅ Works with YouTube Data API
✅ Professional product promo overlays
✅ Sponsored disclosure
✅ CTA → Link in description
❌ NO fake clickable affiliate cards
...
class SlideshowGenerator:
    """
    Generates YouTube Shorts style product promo videos
    """
    ...
    def __init__(self):
        ...
        logger.info(f"FFmpeg path: {self.ffmpeg}")
    ...
    async def generate_slideshow(
        ...
        if not (2 <= len(images) <= 6):
            return {"success": False, "error": "Upload 2–6 images only"}

        session_id = f"vp_{int(asyncio.get_event_loop().time() * 1000)}"
        work_dir = self.temp_dir / session_id
        work_dir.mkdir(parents=True, exist_ok=True)

        for tier in self.QUALITY_TIERS:
            try:
                image_paths = await self._save_images(images, work_dir, tier["resolution"])
                ...
                if product_data:
                    ...
                    image_paths = await self._add_basic_title(image_paths, title, work_dir)

                video_path = await self._render_video(
                    image_paths, duration_per_image, tier, work_dir
                )

                thumb = await self._generate_thumbnail(image_paths[0], work_dir)
                ...
                logger.error(f"{tier['name']} failed: {e}")
                gc.collect()
    ...
    async def _save_images(
        self, images: List[str], work_dir: Path, resolution: Tuple[int, int]
    ) -> List[Path]:
        ...
        for i, img_b64 in enumerate(images):
            ...
            img = self._resize_with_padding(img, resolution)

            path = work_dir / f"img_{i:03d}.jpg"
    ...
    def _resize_with_padding(self, img: Image.Image, target: Tuple[int, int]):
        img_ratio = img.width / img.height
        ...
        canvas.paste(img, ((target[0] - new_w) // 2, (target[1] - new_h) // 2))
        return canvas

    # -------------------------------------------------------------------------
    # OVERLAYS (IMPORTANT PART)
    # -------------------------------------------------------------------------

    async def _add_product_overlays(
        self, image_paths: List[Path], product: Dict, work_dir: Path
    ) -> List[Path]:
        """
        Professional, policy-safe overlays
        """
        ...
        name = product.get("product_name", "Featured Product")[:40]
        ...
        for i, path in enumerate(image_paths):
            ...
            for y in range(200):
                draw.rectangle(
                    [(0, h - y), (img.width, h - y - 1)],
                    fill=(0, 0, 0, int(200 * (y / 200))),
                )

            text = hooks[i % len(hooks)]
            draw.text((40, h - 140), text, font=font_big, fill=(255, 255, 255))

            if brand:
                draw.text((40, h - 80), brand, font=font_small, fill=(200, 200, 200))

            img = Image.alpha_composite(img, overlay).convert("RGB")

            out_path = work_dir / f"overlay_{i:03d}.jpg"
    ...
    async def _add_basic_title(self, image_paths, title, work_dir):
        out = []
        font = ImageFont.load_default()
        ...
        for i, path in enumerate(image_paths):
            ...
            if i == 0:
                draw.text((50, 100), title[:40], fill=(255, 255, 255), font=font)

            out_path = work_dir / f"text_{i:03d}.jpg"
            img.save(out_path)
            img.close()
            out.append(out_path)
    ...
    async def _render_video(
        self, image_paths, duration, tier, work_dir
    ) -> Path:

        filelist = work_dir / "list.txt"
    ...
    async def _generate_thumbnail(self, img_path: Path, work_dir: Path) -> Path:
        img = Image.open(img_path)
        img.thumbnail((640, 360))
        out = work_dir / "thumb.jpg"
