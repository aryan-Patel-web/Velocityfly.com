...
"""
slideshow_generator.py - WITH AFFILIATE PRODUCT OVERLAYS
✅ Adds clickable product cards like YouTube Shorts
✅ Animated text overlays (price, name, brand)
✅ Professional affiliate link promotion
"""
...
class SlideshowGenerator:
    """Generate video slideshows with affiliate product overlays"""
    ...
    async def generate_slideshow(
        ...
        music_style: str = "upbeat",
        aspect_ratio: str = "9:16",
        product_data: Dict = None  # ✅ NEW: Product info for overlay
    ) -> Dict[str, Any]:
        """Generate slideshow with AFFILIATE PRODUCT OVERLAYS"""
        ...
        for tier_index, quality_tier in enumerate(self.QUALITY_TIERS):
            try:
                logger.info(f"🎬 Attempting generation: {quality_tier['name']} (attempt {tier_index + 1}/{len(self.QUALITY_TIERS)})")
                ...
                # Step 2: ✅ ADD PRODUCT OVERLAYS (New feature!)
                if product_data:
                    logger.info("🛍️ Adding affiliate product overlays...")
                    image_paths = await self._add_affiliate_product_overlays(
                        image_paths, product_data, work_dir, quality_tier['resolution']
                    )
                elif add_text:
                    # Fallback to basic text overlays
                    image_paths = await self._add_text_overlays(
                        image_paths, title, language, work_dir
                    )
                ...
                return {
                    ...
                    "local_path": str(video_path),
                    "quality": quality_tier['name'],
                    "has_affiliate_overlay": bool(product_data)
                }
    ...
    # ============================================================================
    # ✅ NEW FEATURE: AFFILIATE PRODUCT OVERLAYS
    # ============================================================================
    
    async def _add_affiliate_product_overlays(
        self,
        image_paths: List[Path],
        product_data: Dict,
        work_dir: Path,
        ...
        """
        ✅ Add YouTube-style affiliate product overlays
        - Clickable product card (top-right corner)
        - Animated text (price, name, brand)
        - Professional styling like the screenshots
        """
        overlay_paths = []
        ...
        product_name = product_data.get("product_name", "Amazing Product")[:40]
        brand = product_data.get("brand", "Brand")[:20]
        price = product_data.get("price", 0)
        discount = product_data.get("discount", "")
        product_url = product_data.get("product_url", "")
        
        # Text variations for animation (changes every image)
        text_variations = [
            f"🔥 {brand} - ₹{int(price)}",
            f"✨ {product_name[:30]}",
            f"💰 Special Price: ₹{int(price)}",
            f"🛍️ {brand} Collection",
            f"⚡ Limited Offer - {discount}" if discount else f"⚡ Buy Now - ₹{int(price)}"
        ]
        ...
        for idx, img_path in enumerate(image_paths):
            try:
                ...
                # ========== 1. PRODUCT CARD (Top-Right Corner) ==========
                card_width = 180
                card_height = 200
                card_x = img.width - card_width - 20
                card_y = 80
                ...
                # "Affiliate" label (like in screenshot)
                label_y = card_y + card_height - 35
                draw.rounded_rectangle(
                    [(card_x + 10, label_y), (card_x + card_width - 10, label_y + 25)],
                    radius=5,
                    fill=(100, 100, 100, 200)
                )
                
                # Draw "Affiliate" text
                affiliate_text = "Affiliate ▼"
                text_bbox = draw.textbbox((0, 0), affiliate_text, font=font_small)
                text_width = text_bbox[2] - text_bbox[0]
                text_x = card_x + (card_width - text_width) // 2
                draw.text((text_x, label_y + 3), affiliate_text, fill=(255, 255, 255), font=font_small)
                
                # Product thumbnail placeholder (you can add actual product image here)
                thumb_size = 120
                thumb_x = card_x + (card_width - thumb_size) // 2
                thumb_y = card_y + 20
                draw.rectangle(
                ...
                # ========== 2. ANIMATED TEXT OVERLAY (Bottom) ==========
                # Choose text variation for this image
                animated_text = text_variations[idx % len(text_variations)]
                
                # Text background (gradient effect)
                text_bg_height = 120
                text_bg_y = img.height - text_bg_height
                
                # Create gradient background
                for i in range(text_bg_height):
                    alpha = int(180 * (i / text_bg_height))  # Fade from transparent to opaque
                    draw.rectangle(
                        [(0, text_bg_y + i), (img.width, text_bg_y + i + 1)],
                        fill=(0, 0, 0, alpha)
                    )
                
                # Main text (animated content)
                text_y = text_bg_y + 30
                
                # Add text shadow for better readability
                for offset in [(-2, -2), (2, 2), (-2, 2), (2, -2)]:
                    draw.text(
                        (50 + offset[0], text_y + offset[1]),
                        animated_text,
                        fill=(0, 0, 0, 150),
                        font=font_large
                    )
                
                # Main text (bright color)
                draw.text((50, text_y), animated_text, fill=(255, 215, 0), font=font_large)
                
                # ========== 3. BRAND WATERMARK (Bottom-Left) ==========
                watermark_text = f"📱 Shop {brand}"
                draw.text((30, img.height - 50), watermark_text, fill=(255, 255, 255, 200), font=font_small)
                
                # ========== 4. CALL-TO-ACTION ==========
                cta_text = "👆 Click to Buy"
                ...
                # CTA background
                draw.rounded_rectangle(
                    [(cta_x - 20, cta_y - 10), (cta_x + cta_width + 20, cta_y + 50)],
                    radius=25,
                    fill=(255, 0, 0, 220)
                )
                
                # CTA text
                draw.text((cta_x, cta_y), cta_text, fill=(255, 255, 255), font=font_medium)
                ...
                # Save with overlay
                overlay_path = work_dir / f"product_overlay_{idx:03d}.jpg"
                img.save(overlay_path, "JPEG", quality=85)
                overlay_paths.append(overlay_path)
                
                logger.info(f"✅ Product overlay added to image {idx + 1}")
                ...
                logger.warning(f"Product overlay failed for image {idx}: {e}")
    ...
    async def _add_text_overlays(
        ...
        """Add basic text overlays (fallback)"""
        overlay_paths = []
