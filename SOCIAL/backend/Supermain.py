...
Fixes: Database initialization, missing methods, OAuth flow
"""
...
# ============================================================================
# UNIFIED DATABASE MANAGER - FIXED VERSION
# ============================================================================
class UnifiedDatabaseManager:
    ...
    def __init__(self, mongodb_uri: str):
        ...
        self.client = None
        self.db = None
        self.connected = False  # ✅ ADDED: Track connection status
    ...
    async def connect(self):
        ...
        try:
            ...
            self.connected = True  # ✅ ADDED: Mark as connected
            logger.info("✅ Unified database connected successfully")
            return True
    ...
    # ✅ ADDED: Missing get_user_by_token method
    async def get_user_by_token(self, token: str) -> Optional[dict]:
        """Get user by JWT token - for authentication"""
        try:
            if not self.connected:
    ...
    async def store_oauth_state(self, state: str, user_id: str, expires_at: datetime) -> dict:
        ...
        try:
            ...
            collection = self.db.oauth_states
            await collection.insert_one({
                "_id": state,  # Use state as ID for easy lookup
                "state": state,
                "user_id": user_id,
                "expires_at": expires_at,
                "created_at": datetime.now()
...
# ✅ ADDED: Optional authentication dependency for OAuth endpoints
async def get_current_user_optional(request: Request) -> Optional[dict]:
    """Get current user if token provided, otherwise None - for OAuth endpoints"""
    try:
...
try:
    ...
    # NOW import functions (they'll use the patched database_manager)
    from main import (
        reddit_oauth_authorize,
        reddit_oauth_callback,
        get_reddit_connection_status,
    ...
    # ✅ FIXED: Reddit OAuth endpoints should NOT require authentication
    # These endpoints are the ENTRY POINT for connecting Reddit
    app.get("/api/oauth/reddit/authorize")(reddit_oauth_authorize)
    app.get("/api/oauth/reddit/callback")(reddit_oauth_callback)
    
    # Regular endpoints (these require auth)
    app.get("/api/reddit/connection-status")(get_reddit_connection_status)
    app.get("/api/reddit/test-connection")(test_reddit_connection)
    app.post("/api/reddit/post")(manual_reddit_post)
    app.post("/api/automation/test-auto-post")(test_auto_post)
